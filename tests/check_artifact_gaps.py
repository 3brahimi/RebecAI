...
Usage:
    python tests/check_artifact_gaps.py --rule-id RULE_ID [--base-dir output]
...
# Add scripts directory to path so validate_step_output is importable
sys.path.insert(0, str(Path(__file__).parent.parent / "skills" / "rebeca_tooling" / "scripts"))
from step_schemas import validate_step_output  # noqa: E402
from output_policy import report_paths  # noqa: E402
...
def check_gaps(rule_id: str, base_dir: Path) -> dict:
    ...
    for step_key, filename, description in REQUIRED_ARTIFACTS:
    ...
    step08_report_missing: list[dict] = []
    if step08_artifact_ok:
        step08_report_missing = _check_step08_report_files(rule_id, base_dir)
        missing.extend(step08_report_missing)

    for step_key, filename, _ in OPTIONAL_ARTIFACTS:
        path = work_dir / filename
        if not path.exists():
            warnings.append({
                "step": step_key,
