...
| `synthesized` | `step05.status == "ok" && issue_class == null && all(c.is_candidate == true && c.mapping_path == "synthesis-agent" for c in candidate_artifacts)` | null | `verified` | invoke `verification_agent` · `{rule_id, model_path: step05.selected_model_path, property_path: step05.selected_property_path, jar_path, output_dir}` |
